    ...
    const togglePeriodStatus = async (period: Period) => {
        ...
        try {
            await fetch('/api/accounting', {
                ...
                body: JSON.stringify({ periodId: period.id, status: newStatus }),
            });
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <Link href="/accounting" className="text-xs font-medium hover:underline" style={{ color: '#A4A9B6', textDecoration: 'none' }}>
                            <ArrowLeft className="w-3.5 h-3.5 inline mr-1" />
                            Accounting &amp; Reporting
                        </Link>
                    </div>
                    <h1 className="section-header">Journal Entries</h1>
                            ...
                            <div className="flex items-center justify-between" style={{ cursor: 'pointer' }}>
                                ...
                                <div className="flex items-center gap-4 text-xs">
                                    ...
                                    <button
                                        ...
                                        className="btn-ghost text-xs"
                                        style={{ marginLeft: 4, color: period.status === 'OPEN' ? '#FA4338' : '#21944E' }}
                                        title={period.status === 'OPEN' ? 'Lock this period' : 'Reopen this period'}
                                    >
                                        {period.status === 'OPEN'
                ...
                <div className="modal-overlay" onClick={() => setAuditDetail(null)}>
                    <div className="modal-content" style={{ maxWidth: 720 }} onClick={(e) => e.stopPropagation()}>
                        ...
                        {auditDetail.amortizationDetails && (
                            <div className="mb-5">
                                <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#A4A9B6' }}>Amortization Schedule</h3>
                        ...
                        {auditDetail.developerSummary && auditDetail.developerSummary.length > 0 && (
                            <div className="mb-5">
                                <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#A4A9B6' }}>Developer Cost Breakdown</h3>
                        ...
                        {auditDetail.auditTrails.length > 0 && (
                            <div>
                                <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#A4A9B6' }}>Ticket-Level Detail</h3>
                                <div className="rounded-lg overflow-hidden border" style={{ borderColor: '#E2E4E9' }}>
                                    <table className="w-full text-xs">
                                        <thead>
                                            <tr style={{ borderBottom: '1px solid #E2E4E9', background: '#F6F6F9' }}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" value={legacyForm.costBasis} onChange={(e) => setLegacyForm({ ...legacyForm, costBasis: e.target.value })} placeholder="0.00" className="form-input" />
                                </div>
                                <div>
                                    <label className="form-label">Accumulated Amortization ($)</label>
                                    <input type="number" value={legacyForm.accumAmort} onChange={(e) => setLegacyForm({ ...legacyForm, accumAmort: e.target.value })} placeholder="0.00" className="form-input" />
                                </div>
                            </div>
                            ...
                            <div style={{ borderTop: '1px solid #E2E4E9', paddingTop: 16 }}>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="date" value={legacyForm.goLiveDate} onChange={(e) => setLegacyForm({ ...legacyForm, goLiveDate: e.target.value })} className="form-input" />
                                    </div>
                                    <div>
                                        <label className="form-label">Useful Life (Months)</label>
                                        <input type="number" value={legacyForm.usefulLifeMonths} onChange={(e) => setLegacyForm({ ...legacyForm, usefulLifeMonths: e.target.value })} placeholder="36" className="form-input" />
                                    </div>
                                </div>
                                    ...
                                    return (
                                        <div className="mt-3 p-3 rounded-lg" style={{ background: '#F6F6F9' }}>
                                            <div className="flex items-center gap-6">
                                                <span className="text-xs" style={{ color: '#A4A9B6' }}>End Date: <span className="font-semibold" style={{ color: '#3F4450' }}>{end.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span></span>
                                                <span className="text-xs" style={{ color: '#A4A9B6' }}>Monthly Depreciation: <span className="font-semibold" style={{ color: '#21944E' }}>${monthlyDepr.toFixed(2)}</span></span>
                                                <span className="text-xs" style={{ color: '#A4A9B6' }}>Remaining Life: <span className="font-semibold" style={{ color: '#4141A2' }}>{months} mo</span></span>
                                            </div>
                                        </div>
                                    );
                                })()}
                                ...
                                <tbody>
                                    {payrollAudit.developers.map((dev, i) => (
                                        <tr key={i} style={{ borderBottom: '1px solid #E2E4E9' }}
                                            ...
                                            <td className="px-4 py-3 text-right font-semibold tabular-nums" style={{ color: Math.abs(dev.delta) < 0.01 ? '#21944E' : '#FA4338' }}>{formatCurrency(dev.delta)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot>
                                    <tr style={{ borderTop: '2px solid #E2E4E9', background: '#F6F6F9' }}>
                                        ...
                                        <td className="px-4 py-3 text-right font-bold tabular-nums" style={{ color: Math.abs(payrollAudit.totals.delta) < 0.01 ? '#21944E' : '#FA4338' }}>{formatCurrency(payrollAudit.totals.delta)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
